...
PJ_XY moll_s_forward (PJ_LP lp, PJ *P) {           /* Spheroidal, forward */
    PJ_XY xy = {0.0,0.0};
...
PJ_LP moll_s_inverse (PJ_XY xy, PJ *P) {           /* Spheroidal, inverse */
    PJ_LP lp = {0.0,0.0};
    struct pj_opaque *Q = static_cast<struct pj_opaque*>(P->opaque);
...
static PJ * setup(PJ *P, double p) {
    ...
    P->inv = PJ_MAKE_KERNEL(moll_s_inverse);
    P->fwd = PJ_MAKE_KERNEL(moll_s_forward);
    return P;
}
