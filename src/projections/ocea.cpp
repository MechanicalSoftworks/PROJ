...
PJ_XY ocea_s_forward (PJ_LP lp, PJ *P) {           /* Spheroidal, forward */
    PJ_XY xy = {0.0,0.0};
...
PJ_LP ocea_s_inverse (PJ_XY xy, PJ *P) {           /* Spheroidal, inverse */
    PJ_LP lp = {0.0,0.0};
    struct pj_opaque *Q = static_cast<struct pj_opaque*>(P->opaque);
...
PJ *PROJECTION(ocea) {
    ...
    if ( pj_param(P->host->ctx, P->host->params, "talpha").i) {
        ...
        alpha   = M_PI + pj_param(P->host->ctx, P->host->params, "ralpha").f;
        lonz = pj_param(P->host->ctx, P->host->params, "rlonc").f;
    ...
    Q->cosphi = cos(phi_p);
    Q->sinphi = sin(phi_p);
    P->inv = PJ_MAKE_KERNEL(ocea_s_inverse);
    P->fwd = PJ_MAKE_KERNEL(ocea_s_forward);
    P->es = 0.;
