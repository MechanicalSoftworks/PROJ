...
PJ_XY eck5_s_forward (PJ_LP lp, PJ *P) {           /* Spheroidal, forward */
    PJ_XY xy = {0.0,0.0};
    (void) P;
...
PJ_LP eck5_s_inverse (PJ_XY xy, PJ *P) {           /* Spheroidal, inverse */
    PJ_LP lp = {0.0,0.0};
    (void) P;
    lp.phi = RYF * xy.y;
...
PJ *PROJECTION(eck5) {
    P->es = 0.0;
    P->inv = PJ_MAKE_KERNEL(eck5_s_inverse);
    P->fwd = PJ_MAKE_KERNEL(eck5_s_forward);
