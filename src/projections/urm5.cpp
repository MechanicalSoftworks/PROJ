...
PJ_XY urm5_s_forward (PJ_LP lp, PJ *P) {           /* Spheroidal, forward */
    PJ_XY xy = {0.0, 0.0};
    struct pj_opaque *Q = static_cast<struct pj_opaque*>(P->opaque);
    double t;

    t = lp.phi = aasin (P->shared_ctx, Q->n * sin (lp.phi));
...
PJ *PROJECTION(urm5) {
    ...
    Q->q3 = pj_param(P->host->ctx, P->host->params, "dq").f / 3.;
    alpha = pj_param(P->host->ctx, P->host->params, "ralpha").f;
    t = Q->n * sin (alpha);
    ...
    P->es = 0.;
    P->inv = nullptr;
    P->fwd = PJ_MAKE_KERNEL(urm5_s_forward);
