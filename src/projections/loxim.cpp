...
PJ_XY loxim_s_forward (PJ_LP lp, PJ *P) {           /* Spheroidal, forward */
    PJ_XY xy = {0.0,0.0};
...
PJ_LP loxim_s_inverse (PJ_XY xy, PJ *P) {           /* Spheroidal, inverse */
    PJ_LP lp = {0.0,0.0};
    struct pj_opaque *Q = static_cast<struct pj_opaque*>(P->opaque);
...
PJ *PROJECTION(loxim) {
    ...
    P->inv = PJ_MAKE_KERNEL(loxim_s_inverse);
    P->fwd = PJ_MAKE_KERNEL(loxim_s_forward);
    P->es = 0.;
