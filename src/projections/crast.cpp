...
PJ_XY crast_s_forward (PJ_LP lp, PJ *P) {           /* Spheroidal, forward */
    PJ_XY xy = {0.0,0.0};
    (void) P;
    lp.phi *= THIRD;
...
PJ_LP crast_s_inverse (PJ_XY xy, PJ *P) {           /* Spheroidal, inverse */
    PJ_LP lp = {0.0,0.0};
    (void) P;
...
PJ *PROJECTION(crast) {
    P->es = 0.0;
    P->inv = PJ_MAKE_KERNEL(crast_s_inverse);
    P->fwd = PJ_MAKE_KERNEL(crast_s_forward);
