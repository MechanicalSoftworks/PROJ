...
PROJ_NOINLINE PJ_XY eqc_s_forward (PJ_LP lp, __global PJ *P) {           /* Spheroidal, forward */
    PJ_XY xy = {0.0,0.0};
...
PROJ_NOINLINE PJ_LP eqc_s_inverse (PJ_XY xy, __global PJ *P) {           /* Spheroidal, inverse */
    PJ_LP lp = {0.0,0.0};
    struct pj_opaque *Q = static_cast<struct pj_opaque*>(P->opaque);
...
PJ *PROJECTION(eqc) {
    ...
    P->inv = PJ_MAKE_KERNEL(eqc_s_inverse);
    P->fwd = PJ_MAKE_KERNEL(eqc_s_forward);
    P->es = 0.;
