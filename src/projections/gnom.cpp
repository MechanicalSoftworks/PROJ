...
PJ_XY gnom_s_forward (PJ_LP lp, PJ *P) {           /* Spheroidal, forward */
    PJ_XY xy = {0.0,0.0};
...
PJ_LP gnom_s_inverse (PJ_XY xy, PJ *P) {           /* Spheroidal, inverse */
    PJ_LP lp = {0.0,0.0};
    struct pj_opaque *Q = static_cast<struct pj_opaque*>(P->opaque);
    double  rh, cosz, sinz;
...
PJ *PROJECTION(gnom) {
    ...
    P->inv = PJ_MAKE_KERNEL(gnom_s_inverse);
    P->fwd = PJ_MAKE_KERNEL(gnom_s_forward);
    P->es = 0.;
