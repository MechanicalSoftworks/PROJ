...
PJ_XY fouc_s_s_forward (PJ_LP lp, PJ *P) {           /* Spheroidal, forward */
    PJ_XY xy = {0.0,0.0};
...
PJ_LP fouc_s_s_inverse (PJ_XY xy, PJ *P) {           /* Spheroidal, inverse */
    PJ_LP lp = {0.0,0.0};
    struct pj_opaque *Q = static_cast<struct pj_opaque*>(P->opaque);
    int i;
...
PJ *PROJECTION(fouc_s) {
    ...
    Q->n1 = 1. - Q->n;
    P->es = 0;
    P->inv = PJ_MAKE_KERNEL(fouc_s_s_inverse);
    P->fwd = PJ_MAKE_KERNEL(fouc_s_s_forward);
    return P;
}
